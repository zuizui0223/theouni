...
**Status:** preferred editorial draft v7; self-contained; cross-domain; not yet journal-formatted
...
- The four interfaces interact, so improving one can leave another limiting or even make it worse.
- The decisive test is prospective: can diagnosis predict which equal-cost measurement expansion helps most?

## Abstract

Ecology and evolution are increasingly measurement-rich. Sensors, remote sensing, bioacoustics, camera traps, genomic assays, environmental DNA and machine-learning pipelines can expand observation volume, precision and downstream performance at rapidly falling marginal cost. Yet these quantities are not universal proxies for evidential progress. We argue that the common problem is **conditionality**: precision is conditional on what was measured, classifier accuracy on what entered the retained record, information gain on which components of the system were learned, and replicate count on which observations share a failure domain. These are useful local quantities, but none alone reports whether a declared ecological or evolutionary claim has become better distinguished. We organize this problem through four interfaces. A biologically relevant difference must be **preserved** by the observation pipeline, the retained measurement must **separate** live alternatives, the separated alternatives must be **relevant** to the scientific target, and repeated support must contain enough **failure diversity** not to depend on one shared point of failure. Missing these properties produces four recurrent diagnoses: too late, same dimension, wrong target and false independence. Because the diagnoses differ, so do their remedies. We propose a prospective measurement-to-evidence screen and a falsifiable programme: before collecting new data, diagnose the limiting interface and test whether the matched equal-cost intervention outperforms quantity-first expansion.
...
This is the key difference from a catalogue of familiar warnings. The question is not whether precision, information gain, replication or classifier accuracy are good metrics. It is **what those metrics are conditional on, and whether that conditioning leaves the declared claim distinguishable**.
...
The shared structure is conditional: every familiar performance metric is calculated after some observational choices, exclusions or losses have already occurred. The design task is therefore to make those conditions explicit and ask whether the declared claim remains distinguishable.
