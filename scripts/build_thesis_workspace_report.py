...
def main() -> None:
    ...
    lines = [
        ...
        "## Chapter registry",
        "",
        "| Order | Unit | Primary owner | Embedded | Status | Writing brief |",
        "|---:|---|---|---|---|---|",
    ]
    ...
    for unit in units:
        ...
        lines.append(
            f"| {unit['order']} | {unit['title']} | {owner} | {embedded} | `{unit['status']}` | `{unit['file']}` |"
        )
    ...
    lines.extend(
        [
            ...
            "## Writing boundary",
            "",
            "Each chapter must preserve its source-owned headline result, one primary forbidden inference, an explicit claim ceiling, and canonical source handoffs. The workspace may coordinate prose and transitions, but it may not absorb source theorem ownership, upgrade bridge modules into independent novelty claims, or infer natural-state adequacy without the Reality-to-Theory admission bridge.",
            "",
        ]
    )

    OUTPUT.write_text("\n".join(lines), encoding="utf-8")
    print(f"Wrote {OUTPUT.relative_to(ROOT)} with {len(units)} writing units and {source_count} source handoffs.")
